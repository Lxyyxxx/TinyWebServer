...
WebServer::WebServer(const int &port, const std::string &user,
                     const std::string &password, const std::string &db_name,
                     const bool &log_write, const bool &opt_linger,
                     const int &trig_mode, const int &sql_num,
                     const int &thread_num, const bool &close_log,
                     const bool &actor_model)
    : m_port(port),
      m_user(user),
      m_password(password),
      m_db_name(db_name),
...
void WebServer::threadPool() {
    // 线程池
    m_pool =
        new threadpool<http_conn>(m_actor_model, m_conn_pool, m_thread_num);
}
...
void WebServer::timer(const int &connfd,
...
void WebServer::dealTimer(util_timer *timer, const int &sockfd) {
...
void WebServer::dealWithRead(const int &sockfd) {
...
void WebServer::dealWithWrite(const int &sockfd) {
    util_timer *timer = users_timer[sockfd].timer;
    // reactor
    if (1 == m_actor_model) {
        if (timer) {
